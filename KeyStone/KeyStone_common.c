 ...
 ******************************************************************************
  ...
  3.Date         : 2012/10/30
    Author       : Zhan
    Modification : Add comments for each functions
******************************************************************************/
	...
	unsigned int multiplier, unsigned int divisor)
{
	...
	/*10. Wait for at least 7 μs based on the reference clock CLKIN (PLL reset time)     */
	for(i=0; i< 2000; i++)
		asm(" nop 5");
	...
	unsigned int multiplier, unsigned int outputDivisor)
{
	...
	BWADJ value must be set to ((PLLM + 1) >> 1) – 1)*/
...
*****************************************************************************/
void EDMA_channel_TC_cfg (CSL_TpccRegs*  EDMACCRegs, 
	Uint32 uiChannel, Uint32 uiTC)
{
	EDMACCRegs->TPCC_DMAQNUM[uiChannel/8] = 
		(EDMACCRegs->TPCC_DMAQNUM[uiChannel/8]&(~(0xF<<((uiChannel&7)*4))))
		|(uiTC<<((uiChannel&7)*4));
}
/*****************************************************************************
...
*****************************************************************************/
void EDMA_TC_priority_cfg(CSL_TpccRegs*  EDMACCRegs, 
	Uint32 uiPri, Uint32 uiTC)
{
	EDMACCRegs->TPCC_QUEPRI= 
		(EDMACCRegs->TPCC_QUEPRI&(~(0xF<<((uiTC&3)*4))))
		|(uiPri<<((uiTC&3)*4));
}
/*****************************************************************************
 Prototype    : EDMA_init
	...
	unsigned int byteCount, EDMA_CC_Channel_Num CC_channel, DMA_Wait wait)
{
	...
	if(byteCount) 	//the reminding must be multiple of 128 bytes
	{
		EDMACCRegs= gpEDMA_CC_regs[CC_channel>>16];
		uiChannel = CC_channel&0xFF;
		...
		EDMACCRegs->PARAMSET[uiChannel].A_B_CNT= CSL_EDMA3_CNT_MAKE(128, byteCount/128);
	...
	Uint32 byteCount, Uint32 AccessPermisionMask)
{
	...
	{
		printf("L2 memory protection start Address 0x%x is not a local address\n",
			startAddress);
	...
	{
		printf("L2 memory protection start Address 0x%x does not align to page boundary\n",
			startAddress);
	...
	{
		printf("L2 memory protection section size 0x%x is not multiple of page size\n",
			byteCount);
...
/*****************************************************************************
 ...
 Return Value : 
  ...
  2.Date         : 2012/10/28
    Author       : Brighton Feng
    Modification : Modify the function for common usage
*****************************************************************************/
...
void KeyStone_MPU_interrupts_enable()
{
	...
	for(i=0; i<4; i++)
	{
	    gpMPU_regs[i]->FAULT_CLEAR = 1;
	    gpMPU_regs[i]->INT_ENABLED_STATUS_CLEAR = 
	    	CSL_MPU_INT_ENABLED_STATUS_CLEAR_ENABLED_ADDR_ERR_MASK
	    	|CSL_MPU_INT_ENABLED_STATUS_CLEAR_ENABLED_PROT_ERR_MASK;
		gpMPU_regs[i]->INT_ENABLE = CSL_MPU_INT_ENABLE_ADDR_ERR_EN_MASK 
	    	|CSL_MPU_INT_ENABLE_PROT_ERR_EN_MASK;                
	}
...
void LL2_EDC_scrub(Uint32 uiByteCnt)
{
	...
	uiByteCnt &= 0xFFFFFFF0;	//size must be multiple of 128 bits
...
char * LL2_EDC_victim_err_str="L2 victims";
...
/*****************************************************************************
 ...
 Return Value : 
  ...
  2.Date         : 2012/10/20
    Author       : Zhan
    Modification : Add peripheral MPU support
...
/*****************************************************************************
 ...
 Return Value : 
  ...
  2.Date         : 2012/9/18
    Author       : Zhan
    Modification : Add memory protection processing

  3.Date         : 2012/10/28
    Author       : Brighton Feng
    Modification : Add edc processing
*****************************************************************************/
...
/*****************************************************************************
 Prototype    : KeyStone_print_device_info
...
*****************************************************************************/
void KeyStone_print_device_info()
{
	Uint32 * DieID= (Uint32 *)&gpBootCfgRegs->DIE_ID_REG0;

	printf("Die ID= 0x%08x, 0x%08x, 0x%08x, 0x%08x\n", DieID[0], DieID[1], DieID[2], DieID[3]);
	printf("JTAG ID= 0x%08x, 0x%08x\n", gpBootCfgRegs->DEVICE_ID_REG0, gpBootCfgRegs->DEVICE_ID_REG1);
	printf("DEVSTAT= 0x%08x\n", gpBootCfgRegs->BOOT_REG0);
}
	...
	unsigned int uiByteCount, unsigned int uiMaxFails, int iStep)
{
	...
	for(i=0; i<uiByteCount/8; i++)
	{
        ...
        {
			printf("Memory address Test fails at 0x%8x, Write 0x%16llx, Readback 0x%16llx\n", 
	...
	unsigned int uiMaxFails, int iStep)
{
	...
	for(i=0; i<uiByteCount/8; i++)
	{
        ...
        {
			printf("Memory pattern filling Test fails at 0x%8x, Write 0x%16llx, Readback 0x%16llx\n", 
    ...
    for (i = 0; i < uiNumBitShift; i++)
    {
        for(j=0; j<uiBusWidth_64bit; j++)
        {
	        ...
	        *ulpAddressPointer = ulBitMask;
			...
			{
				printf(" Data bit walking Test fails at 0x%8x, Write 0x%16llx, Readback 0x%16llx\n", (unsigned int)ulpAddressPointer, (ulBitMask), ulReadBack);
	            uiFailCount++;
			}	            
	...
	unsigned int uiNumBytes, unsigned int uiMAU_bytes)
{
    ...
    volatile unsigned char ucReadBack;
    volatile unsigned short usReadBack;
	...
	{//test address bit 1
        ...
        *(unsigned short *)(uiBaseAddress+uiNumBytes-4) = 0xFFFC;
		usReadBack= *(unsigned short *)(uiBaseAddress+uiNumBytes-1);
